...
fluidPage(
  ...
  fluidRow(
    ...
    p("Australian state shapefiles (State and Territories 2021) were obtained from the Australian Bureau of Statistics: ",  tags$a(href = "https://www.abs.gov.au/statistics/standards/australian-statistical-geography-standard-asgs-edition-3/jul2021-jun2026/access-and-downloads/digital-boundary-files", "abs.gov.au")),
    p("Body polygon shapefiles were obtained from Jose Fernandez's GitHub: ", tags$a(href = "https://github.com/josedv82/bdgramR?tab=MIT-2-ov-file", "github.com/josedv82")),
    ),
    
    column(6,
      style = "border: 2px solid #8db8fc; padding: 50px",
      h4("Authors"),
...
Where are Shark Attacks Occurring? {data-navmenu="Menu"}
=====================================
...
shark_attacks_fixed <- shark_attacks %>%
  ...
  mutate(Victim.activity = case_when(Victim.activity == "boarding" ~ "Boarding",
                                   ...
                                   Victim.activity == "motorised boating" ~ "Boating",
...
```{r}
#loading in the sf data, reading in the Australian state shapefiles
load("aus_shp.RData")
...
shark_attacks_fixed_latlon <- shark_attacks_fixed %>%
...
radioButtons("response_var_map", label = h3("Attack Characteristic:"),
   choices = list("Victim Age" = "Victim.age", "Provoked?" = "Provoked.unprovoked", "Victim Gender" = "Victim.gender",
...
Row {data-height=30}
-----------------------------------------------------------------------
Explore this map of shark attacks across Australia to better understand where certain types of attacks have occured, by selecting different attack characteristics and hovering over different points to learn more about the attacks!

Row {data-height=30}
-----------------------------------------------------------------------
Through looking at the maps, you can see that Queensland and New South Wales appear to have a much higher density of points along their coast. Queensland and Western Australia also appear to have the longest coasts, which might impact how many shark attacks occur in those states. Respective state populations could also influence the number of attacks and the proportion of different attack characteristics occurring in each state. There are also certain areas with higher clustering of certain characteristics. For example, there are patches of Western Australian where the attacks occurred with women and patches where the attacks occurred with men. Further analysis could explore if apparent clustering is statistically significant.  
...
renderPlotly({
  shark_attacks_sf <- shark_attacks_sf %>%
      mutate(!!sym(input$response_var_map) := factor(!!sym(input$response_var_map),
        levels = if (input$response_var_map == "Victim.injury") {
          ...
          c("Unprovoked", "Provoked")
        } else if (input$response_var_map == "Victim.activity") {
          c("Swimming", "Boarding", "Snorkeling", "Spearfishing","Boating", "Diving", "Fishing",  "Standing in Water", "Other")
  ...
  map <- ggplot()+
    ...
    geom_sf(data = shark_attacks_sf, aes(fill = .data[[input$response_var_map]], text = paste0("Date: ", paste(Incident.month, Incident.year, sep = "/"),
          ...
          size = 1.5) +
        scale_fill_manual(values = c("Fatal" = "#a50f15", "Injured" = "#ef3b2c", "Uninjured" = "#fcbba1", "Ocean/Pelagic" = "#343d6b", "Island Open Ocean" = "#045a8d", "Estuary/Harbour" = "#2b8cbe", "Coastal" = "#74a9cf",  "River" = "#a6bddb", "Other" = "#969696","Provoked" = "#8c3a40", "Unprovoked" = "#343d6b", "Swimming" = "#a6cee3", "Boarding" = "#1f78b4", "Spearfishing" = "#b2df4a", "Snorkeling" = "#33a02c", "Diving" = "#fb9a99", "Boating" = "#e31a1c", "Standing in Water" = "#fdbf6f", "Fishing" = "#ff7f00", "Other" = "#cab2d6",
          "Major Lacerations" = "#a50f15", "Minor Lacerations" = "#de2d26", "Punctures" = "#fb6a4a" , "Surface Wound" = "#fc9272", "Abrasion" = "#fcbba1", "Other: Teeth Marks" = "#fee5d9", "Male" = "#2b8cbe", "Female" = "#fb6f99", is.na = "black"),
          labels = c("Fatal", "Injured", "Uninjured", "Ocean/Pelagic", "Island Open Ocean", "Estuary/Harbour", "Coastal", "River", "Other", "Unprovoked", "Provoked", "Swimming", "Boarding", "Spearfishing", "Snorkeling", "Diving", "Boating", "Standing in Water", "Fishing", "Other", "Major Lacerations", "Minor Lacerations", "Punctures", "Surface Wound", "Abrasion", "Other: Teeth Marks", "Male", "Female"))+
      interactive_theme  +
...
This page highlights the number of shark attacks recorded by month of the year, and time of the day. You can use the button on the sidebar to switch between graphs. As one may expect, more shark attacks occur at times when more people are swimming. This is true for months of the year, where December through February, which are summer months in the southern hemisphere, have the most recorded incidents. Additionally, more attacks occur during the day than at night, with a sharp decline after 5:00 pm. By comparing the black total lines with the other lines representing 50-year blocks, it is clear that both of these trends have remained consistent over time. Perhaps instead of going for an afternoon swim during the crowded summer months, try enjoying a cool, beautiful fall sunset on the beach.
...
renderPlotly({
  if(input$time_button == "month")
    ...
    p_time <- ggplot(sharks_time_hour, aes(x = Time.of.incident, y = count_hour, color = as.factor(Incident.year))) +
      geom_point(aes(text = paste("Time:", sapply(Time.of.incident, convert_to_time), "-", sapply(Time.of.incident, convert_to_time), "<br>Years:", Incident.year, "<br>Number of Recorded Attacks:", count_hour))) +
...
shark_attacks_fixed <- shark_attacks_fixed %>%
  group_by(Year) %>%
...
shark_attacks_timeline <- shark_attacks_fixed %>%
  mutate(Victim.age = as.ts(Victim.age)) %>%
  mutate(Incident.month = as.character(Incident.month)) %>%
  mutate(Incident.year.year = as.character(Incident.year)) %>%
...
shark_attacks_count <- shark_attacks_timeline %>%
  ...
  mutate(Incident.year = as.Date(paste0(Incident.year, "-01-01"))) %>%
  mutate(Incident.year.year = format(Incident.year, "%Y"))
...
#Victim.injury
shark_attacks_proportions_Victim.injury <- shark_attacks_fixed %>%  #  Start with the cereal data.frame
...
#Site.category
shark_attacks_proportions_Site.category <- shark_attacks_fixed %>%  #  Start with the cereal data.frame
...
#Provoked.unprovoked
shark_attacks_proportions_Provoked.unprovoked <- shark_attacks_fixed %>%  #  Start with the cereal data.frame
...
#Victim.activity
shark_attacks_proportions_Victim.activity <- shark_attacks_fixed %>%  #  Start with the cereal data.frame
...
#Victim.gender
shark_attacks_proportions_Victim.gender <- shark_attacks_fixed %>% #  Start with the data.frame
...
#Injury.severity
shark_attacks_proportions_Injury.severity <- shark_attacks_fixed %>%  #Start with the cereal data.frame
...
renderPlotly({
  if(input$response_var %in% barchart){
    ...
    dataframe_barchart_ordered <- dataframe_barchart() %>%
      mutate(!!sym(input$response_var) := factor(!!sym(input$response_var),
        levels = if (input$response_var == "Victim.injury") {
          ...
          c("Male", "Female")
        } else if (input$response_var == "Total") {
          c("Total")
        } else {
          NULL
        }
    ...
    dataframe_barchart_ordered$total_group <- "Total"
         ...
         labs(x = "Years",
              ...
              title = list(text = paste0(paste0(paste0(ifelse(input$prop_count == "prop", "Proportion", "Count"),
               ...
               # subtitle = "Error Bars show 95% Confidence Interval\nDates are by 50 Years except for 2000-2024",
                fill = ifelse(input$response_var == "Victim.injury", "Victim Injuries", ifelse(input$response_var == "Site.category", "Site Categories", ifelse(input$response_var == "Provoked.unprovoked", "Provoked?", ifelse(input$response_var == "Victim.activity", "Victim Activities", ifelse(input$response_var == "Injury.severity", "Injurity Severity", ifelse(input$response_var == "Victim.gender", "Victim Gender", "Total")))))),
              color = ifelse(input$response_var == "Victim.injury", "Victim Injuries", ifelse(input$response_var == "Site.category", "Site Categories", ifelse(input$response_var == "Provoked.unprovoked", "Provoked?", ifelse(input$response_var == "Victim.activity", "Victim Activities", ifelse(input$response_var == "Injury.severity", "Injurity Severity", ifelse(input$response_var == "Victim.gender", "Victim Gender", "Total"))))))) +
      scale_color_manual(values = c("Fatal" = "#a50f15", "Injured" = "#ef3b2c", "Uninjured" = "#fcbba1", "Ocean/Pelagic" = "#343d6f", "Island Open Ocean" = "#045a8d", "Estuary/Harbour" = "#2b8cbe", "Coastal" = "#74a9cf",  "River" = "#a6bddb", "Other" = "grey40","Provoked" = "#8c3a40", "Unprovoked" = "#343d6b", "Swimming" = "#a6cee3", "Boarding" = "#1f78b4", "Spearfishing" = "#b2df4a", "Snorkeling" = "#33a02c", "Diving" = "#fb9a99", "Boating" = "#e31a1c", "Standing in Water" = "#fdbf6f", "Fishing" = "#ff7f00", "Other" = "#cab2d6",
          "Major Lacerations" = "#a50f15", "Minor Lacerations" = "#de2d26", "Punctures" = "#fb6a4a" , "Surface Wound" = "#fc9272", "Abrasion" = "#fcbba1", "Other: Teeth Marks" = "#fee5d9", "Male" = "#2b8cbe", "Female" = "#fb6f99", "Total" = "black"),
           labels = c("Fatal", "Injured", "Uninjured", "Ocean/Pelagic", "Island Open Ocean", "Estuary/Harbour", "Coastal", "River", "Other","Provoked", "Unprovoked", "Swimming", "Boarding", "Spearfishing", "Snorkeling", "Diving", "Boating", "Standing in Water", "Fishing", "Other", "Major Lacerations", "Minor Lacerations", "Punctures", "Surface Wound", "Abrasion", "Other: Teeth Marks", "Male", "Female", "Total"))+

       scale_fill_manual(values = c("Fatal" = "#a50f15", "Injured" = "#ef3b2c", "Uninjured" = "#fcbba1", "Ocean/Pelagic" = "#343d6b", "Island Open Ocean" = "#045a8d", "Estuary/Harbour" = "#2b8cbe", "Coastal" = "#74a9cf",  "River" = "#a6bddb", "Other" = "grey40","Provoked" = "#8c3a40", "Unprovoked" = "#343d6b", "Swimming" = "#a6cee3", "Boarding" = "#1f78b4", "Spearfishing" = "#b2df4a", "Snorkeling" = "#33a02c", "Diving" = "#fb9a99", "Boating" = "#e31a1c", "Standing in Water" = "#fdbf6f", "Fishing" = "#ff7f00", "Other" = "#cab2d6",
          "Major Lacerations" = "#a50f15", "Minor Lacerations" = "#de2d26", "Punctures" = "#fb6a4a" , "Surface Wound" = "#fc9272", "Abrasion" = "#fcbba1", "Other: Teeth Marks" = "#fee5d9", "Male" = "#2b8cbe", "Female" = "#fb6f99", "Total" = "black"),
          labels = c("Fatal", "Injured", "Uninjured", "Ocean/Pelagic", "Island Open Ocean", "Estuary/Harbour", "Coastal", "River", "Other","Provoked", "Unprovoked", "Swimming", "Boarding", "Spearfishing", "Snorkeling", "Diving", "Boating", "Standing in Water", "Fishing", "Other", "Major Lacerations", "Minor Lacerations", "Punctures", "Surface Wound", "Abrasion", "Other: Teeth Marks", "Male", "Female", "Total"))+
      interactive_theme +
  ...
  if (input$error_bar & input$prop_count == "prop") {
    barchart_plot <- barchart_plot +
      geom_errorbar(data = dataframe_barchart_ordered,
                    ...
                    position = position_dodge(0.1)) + # position_dodge(0.9)) #position = "dodge") #position_dodge(preserve = "single") preserve = "single", padding = 0.1
          geom_point(aes(x = Year))

  }
    
   # Include the total count line and tooltips
    if (input$prop_count == "count") {
    
      
  barchart_plot <- barchart_plot +
     geom_line(
      data = dataframe_barchart_ordered,
      aes(
        x = Year,
        y = !!sym(paste0("total_", input$response_var)),
        group = "Total", color = "Total", fill = "Total")) +
    geom_point(
      data = dataframe_barchart_ordered,
      aes(x = Year,
          y = !!sym(paste0("total_", input$response_var)),
         text = paste0("Total Count: ", !!sym(paste0("total_", input$response_var))),
          color = "Total", fill = "Total", group = "Total")) 
...
"<br>Years: ", dataframe_barchart_ordered$Year),
    ...
    lineplot_plot <- ggplot(data = dataframe_time(),
        ...
        geom_smooth(method = "loess", alpha = 0.2, color = alpha("#a50f15", 0.3), figure = 'y~x', aes(text = tooltip_text_date)) +
        labs(x = "Date",
       ...
       scale_x_date(date_breaks = "10 years", 
                 date_labels = "%Y",
                 limits = c(as.Date("1802-01-01"), as.Date("2024-01-01"))) +
    ...
    if (input$response_var == "attack_count") {
      tooltip_text_date = paste0("<br>Attack Count: ", shark_attacks_count$attack_count,
                                                      "<br>Year: ", shark_attacks_count$Incident.year.year)
    } else {
       tooltip_text_date = paste0("<br>Victim Age: ", shark_attacks_timeline$Victim.age,
                                                      "<br>Year: ", shark_attacks_timeline$Incident.year.year)
    }


        # Add tooltip_text to the plot
    lineplot_plot <- lineplot_plot + aes(text = tooltip_text_date) +
      geom_smooth(aes(text = NULL), method = "loess", alpha = 0.2, color = alpha("blue", 0.3), figure = 'y~x') 
