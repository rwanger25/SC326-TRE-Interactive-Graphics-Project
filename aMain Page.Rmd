...
renderPlotly({
  if(input$response_var %in% barchart){
    ...
    barchart_plot <- ggplot(dataframe_barchart_ordered,
      ...
      # geom_line(aes_string(x = "Year", y = ifelse(input$prop_count == "count", sum(paste0("count_", input$response_var)), NULL)), color = "black")+
      # geom_point(aes(x = Year))+
         labs(x = "Years",
  ...
  if (input$error_bar & input$prop_count == "prop") {
    barchart_plot <- barchart_plot +
      geom_errorbar(data = dataframe_barchart_ordered,
                    ...
                    position = position_dodge2(preserve = "single", padding = 0.1)) + # position_dodge(0.9)) #position = "dodge") #position_dodge(preserve = "single")
          geom_point(aes(x = Year))
...
"<br>Years: ", dataframe_barchart_ordered$Year),
    ...
    lineplot_plot <- lineplot_plot + aes(text = tooltip_text_date) +
      geom_smooth(method = "loess", alpha = 0.2, color = alpha("blue", 0.3), figure = 'y~x') 
