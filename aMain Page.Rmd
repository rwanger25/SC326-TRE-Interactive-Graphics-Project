...
renderPlotly({
  if(input$response_var %in% barchart){
    ...
    barchart_plot <- ggplot(dataframe_barchart_ordered) +
      geom_line(aes_string(x = "Year", y = ifelse(input$prop_count == "prop", paste0("proportion_", input$response_var), ifelse(input$prop_count == "count", paste0("count_", input$response_var), NULL)),  fill = input$response_var, group = input$response_var, color = input$response_var))+
      geom_point(aes_string(x = "Year", y = ifelse(input$prop_count == "prop", paste0("proportion_", input$response_var), ifelse(input$prop_count == "count", paste0("count_", input$response_var), NULL)),  fill = input$response_var, color = input$response_var))+

      # ggplot(dataframe_barchart_ordered,
  ...
  barchart_plot <- barchart_plot +
    ...
    geom_line(
      ...
      aes(
        x = Year,
        y = !!sym(paste0("total_", input$response_var)),
        group = !!sym(input$response_var), color = "Total", fill = "Total"
      ))
}
