...
renderPlot({
  plot <- ggplot(dataframe(), aes_string(y = paste0("proportion_", input$response_var), fill = input$response_var)) +
  ...
  # if (input$error_bar) {
  #   plot <- plot + geom_errorbar(aes_string(ymin = paste0("lower_", input$response_var), ymax = paste0("upper_", input$response_var)), color = "red", position = "dodge") #position_dodge(width = 0.9)
  # } 
  
  # Add error bars if selected
  if (input$error_bar) {
    validate(
      need(paste0("lower_", input$response_var) %in% names(df), "Lower bound column missing in data!"),
      need(paste0("upper_", input$response_var) %in% names(df), "Upper bound column missing in data!")
    )
    plot <- plot +
