...
fluidPage(
  ...
  fluidRow(
    column(12,
           ...
           p("Click the buttons along the top bar to start exploring different graphs. Please be aware some graphs might take longer to render")
    )
  ),
...
renderGauge({
  ...
  gauge(rate, min = 0, max = 100, symbol = '%', gaugeSectors( # render the gauge
    success = c(0, 14), warning = c(15, 29), danger = c(30, 100)
  ))
})
```
    ...
    if(input$provoke_EL == "both") { # and they both provoke and unprovoke
      ...
      bubble <- ggplot(shark_final, aes(x= attack_species, y= reorder(Shark.common.name, attack_species), color = percent, text = text)) +
        ...
        labs(x = "Number of Attacks per Species",
             y = "Shark Common Names",
             color = "Percent fatality",
             title = paste("Which Shark Species Attack is Most Common and Fatal?-", str_to_title(input$provoke_EL))) +
        interactive_theme
  ...
  if (input$shark_species_EL == "all") { #if the shark species is all
    if(input$provoke_EL == "both") { # and they both provoke and unprovoke
      ...
      ggplot(fatal_shark, aes(fatal, Shark.length.m, fill = fatal)) +
        ...
        scale_fill_manual(values = c("Fatal" = "#8c3a40", "Not fatal" = "#343d6b")) +
        scale_color_manual(values = c("Fatal" = "#8c3a40", "Not fatal" = "#343d6b")) +
        geom_jitter( aes(color = fatal)) +
        theme(legend.position = "none") +
        labs(x = "Fatal or Not Fatal Attack",
             y = "Shark Length (m)",
             title = "Are Longer Sharks More Fatal?",
             fill = "Fatal or Not Fatal",
             color = "Fatal or Not Fatal") +
        interactive_theme
      ...
      ggplot(fatal_shark, aes(fatal, Shark.length.m, fill = fatal)) +
        ...
        labs(x = "Fatal or Not Fatal Attack",
             y = "Shark Length (m)",
             title = paste("Are Longer Sharks More Fatal?-", str_to_title(input$provoke_EL)),
             fill = "Fatal or Not Fatal",
             color = "Fatal or Not Fatal") +
        interactive_theme
    } 
    ...
    if(input$provoke_EL == "both") { # and they both provoke and unprovoke
      ...
      ggplot(fatal_shark, aes(fatal, Shark.length.m, fill = fatal)) +
        ...
        labs(x = "Fatal or Not Fatal Attack",
             y = "Shark Length (m)",
             title = paste("Are Longer", str_to_title(input$shark_species_EL), "More Fatal?"),
             fill = "Fatal or Not Fatal",
             color = "Fatal or Not Fatal") +
        interactive_theme
      ...
      ggplot(fatal_shark, aes(fatal, Shark.length.m, fill = fatal)) +
        ...
        labs(x = "Fatal or Not Fatal Attack",
             y = "Shark Length (m)",
             title = paste("Are Longer", str_to_title(input$shark_species_EL), "More Fatal?-", str_to_title(input$provoke_EL)),
             fill = "Fatal or Not Fatal",
             color = "Fatal or Not Fatal") +
        interactive_theme
    } 
...
-----------------------------------------------------------------------
  ...
  Row 
-----------------------------------------------------------------------
  ### Body Part Injuries by Age Range
...
renderPlot({
  ...
  if (input$bar_EL == "all") { #if it's all barchart
    ...
    body_bar <- ggplot(body_total_age, aes(x = reorder(overall, Number_injury), y = Number_injury)) +
      geom_col(aes(fill = Number_injury)) +
...
shark_attacks <- read.csv("Australian Shark-Incident Database Public Version.csv")
blurb_timeline <- read.csv("timeline blurb.csv")
...
renderTable({
  blurb_timeline %>%
    filter(name == input$response_var) %>% 
    select(Takeaways)
...
Row {data-height=30}
-----------------------------------------------------------------------
  This map is to better understand where certain types of attacks appear to occur. Click different attack characteristics to see where the type of attacks are occurring. Hover over different points to learn more about the attacks!
...
renderPlotly({
  shark_attacks_sf <- shark_attacks_sf %>%
    mutate(!!sym(input$response_var_map) := factor(!!sym(input$response_var_map), 
                                                   levels = if (input$response_var_map == "Victim.injury") {
                                                     c("Fatal", "Injured", "Uninjured")
                                                   } else if (input$response_var_map == "Site.category") {
                                                     c("Ocean/Pelagic", "Island Open Ocean", "Estuary/Harbour", "Coastal", "River", "Other")
                                                   } else if (input$response_var_map == "Provoked.unprovoked") {
                                                     c("Provoked", "Unprovoked")
                                                   } else if (input$response_var_map == "Victim.activity") {
                                                     c("Boarding", "Boating", "Diving", "Fishing", "Motorised Boating", "Other", "Snorkeling", "Spearfishing", "Standing in Water", "Swimming")
                                                   } else if (input$response_var_map == "Injury.severity") {
                                                     c("Major Lacerations", "Minor Lacerations", "Punctures", "Surface Wound", "Abrasion", "Other: Teeth Marks")
                                                   } else if (input$response_var_map == "Victim.gender") {
                                                     c("Male", "Female")
                                                   } else {
                                                     NULL
                                                   }
  ...
  map <- ggplot()+
    ...
    labs(x = "", y = "", 
         title = list(text = 
                        paste0("Location", ifelse(input$response_var_map == "Victim.injury", " of Victim Injury Type across Australia", ifelse(input$response_var_map == "Site.category", " of Shark Attack Site Categories Over Time", ifelse(input$response_var_map == "Provoked.unprovoked", " of Provoked vs. Unprovoked Shark Attacks Over Time", ifelse(input$response_var_map == "Victim.activity", " of Victim Activities Over Time", ifelse(input$response_var_map == "Injury.severity", " of Injurity Severity Over Time", ifelse(input$response_var_map == "Victim.gender", " of Victim Genders Over Time", "")))))))),
         fill = ifelse(input$response_var_map == "Victim.injury", "Victim Injuries", ifelse(input$response_var_map == "Site.category", "Site Categories", ifelse(input$response_var_map == "Provoked.unprovoked", "Provoked?", ifelse(input$response_var_map == "Victim.activity", "Victim Activities", ifelse(input$response_var_map == "Injury.severity", "Injurity Severity", ifelse(input$response_var_map == "Victim.gender", "Victim Gender", ""))))))) +
    ...
    geom_sf(data = shark_attacks_sf, aes(fill = .data[[input$response_var_map]], text = paste0("Date: ", paste(Incident.month, Incident.year, sep = "/"),
            ...
            size = 1.5) +
    scale_fill_manual(values = c("Fatal" = "#a50f15", "Injured" = "#ef3b2c", "Uninjured" = "#fcbba1", "Ocean/Pelagic" = "#343d6b", "Island Open Ocean" = "#045a8d", "Estuary/Harbour" = "#2b8cbe", "Coastal" = "#74a9cf",  "River" = "#a6bddb", "Other" = "#969696","Provoked" = "#8c3a40", "Unprovoked" = "#343d6b", "Swimming" = "#a6cee3", "Boarding" = "#1f78b4", "Spearfishing" = "#b2df8a", "Snorkeling" = "#33a02c", "Diving" = "#fb9a99", "Boating" = "#e31a1c", "Standing in Water" = "#fdbf6f", "Fishing" = "#ff7f00", "Other" = "#cab2d6",
                                 "Major Lacerations" = "#a50f15", "Minor Lacerations" = "#de2d26", "Punctures" = "#fb6a4a" , "Surface Wound" = "#fc9272", "Abrasion" = "#fcbba1", "Other: Teeth Marks" = "#fee5d9", "Male" = "#a6cee3", "Female" = "#fb9a99", is.na = "black"),
                      labels = c("Fatal", "Injured", "Uninjured", "Ocean/Pelagic", "Island Open Ocean", "Estuary/Harbour", "Coastal", "River", "Other","Provoked", "Unprovoked", "Swimming", "Boarding", "Spearfishing", "Snorkeling", "Diving", "Boating", "Standing in Water", "Fishing", "Other", "Major Lacerations", "Minor Lacerations", "Punctures", "Surface Wound", "Abrasion", "Other: Teeth Marks", "Male", "Female"))+
    interactive_theme
